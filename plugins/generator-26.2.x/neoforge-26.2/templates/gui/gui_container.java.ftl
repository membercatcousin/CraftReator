...
public class ${name}Menu extends AbstractContainerMenu implements ${JavaModName}Menus.MenuAccessor {
	...
	<#if data.type == 1>
		...
		<#-- #47997: ResourceHandlerSlot#getItem hands back a copy, so the stacking branch
		     mutating `target` in place needs an explicit write-back. Anchor each write-back
		     on the setCount() above it: vanilla has a third slot.setChanged() in the
		     empty-slot branch, where `target` is the EMPTY stack read before insertion, and
		     patching that one wipes the item setByPlayer just placed. -->
		@Override ${mcc.getMethod("net.minecraft.world.inventory.AbstractContainerMenu", "moveItemStackTo", "ItemStack", "int", "int", "boolean")
			.replace("target.setCount(totalStack);", "target.setCount(totalStack); slot.set(target);")
			.replace("target.setCount(maxStackSize);", "target.setCount(maxStackSize); slot.set(target);")
