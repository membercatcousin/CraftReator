...
<#function mappedMCItemToIngredient mappedBlock>
    <#if mappedBlock.getUnmappedValue().startsWith("TAG:")>
        <#return "Ingredient.of(HolderSet.emptyNamed(BuiltInRegistries.ITEM, TagKey.create(Registries.ITEM, Identifier.parse(\"" + mappedBlock.asTagEntry() + "\"))))">
    <#elseif mappedBlock.getMappedValue(1).startsWith("#")>
        <#return "Ingredient.of(HolderSet.emptyNamed(BuiltInRegistries.ITEM, TagKey.create(Registries.ITEM, Identifier.parse(\"" + mappedBlock.getMappedValue(1).replace("#", "") + "\"))))">
    <#else>
...
<#function containsAnyOfBlocks elements blockToCheck>
    ...
    <#if tags?has_content>
        ...
        <#list tags as tag>
        	<#assign retval += "TagKey.create(Registries.BLOCK, Identifier.parse(\"" + tag + "\"))">
