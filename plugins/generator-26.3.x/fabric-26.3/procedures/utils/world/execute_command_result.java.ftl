private static String executeCommandGetResult(LevelAccessor world, Vec3 pos, String command) {
	...
	if (world instanceof ServerLevel level) {
		...
		level.getServer().getCommands().performPrefixedCommand(new CommandSourceStack(dataConsumer, pos, Vec2.ZERO, level, LevelBasedPermissionSet.OWNER, level.getServer(), null), command);
	}
